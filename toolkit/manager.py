...
class MLSystemManager:
    ...
    def main(self):
        ...
        if eval_method == "training":
        ...
        elif eval_method == "creativity_project":
            # load the model
            learnerVer = self.get_learner(learner_name)
            learnerVir = self.get_learner(learner_name)
            learnerSet = self.get_learner(learner_name)
            ...
            labels_iris = data.col(data.cols-1)
            for i in range(len(labels_iris)):
                if(labels_iris[i] != index_change and label_index_iver == 0):
                    label_index_iver = i
                    index_change = labels_iris[i]
                    irisSetosa = Matrix(data, 0, 0, i, data.cols)
                    num_rows = i

                elif labels_iris[i] != index_change and label_index_ivir == 0:
                    index_change = labels_iris[i]
                    print("I: ", i)
                    print("num_rows", num_rows)
                    irisVirginica = Matrix(data, i, 0, i - num_rows, data.cols)
                    num_rows = i
            irisVersicolor = Matrix(data, label_index_iver, 0, data.rows - num_rows, data.cols)
            # print("Iris Setosa: ", irisSetosa.data)
            # print("Iris Versicolor: ", irisVersicolor.data)
            # print("Iris Virginica: ", irisVirginica.data)


            iSVe = copy.deepcopy(irisSetosa)
            iSVe.data.extend(irisVersicolor.data)
            iSVeLabels = iSVe.col(iSVe.cols-1)
            print(iSVeLabels)
            iSVe.data = iSVe.data[0:len(iSVe.data)]
            print(iSVe.data)

            iVV = copy.deepcopy(irisVersicolor)
            iVV.data.extend(irisVirginica.data)
            # print(iVV.data)
            iVVLabels = iVV.col(iVV.cols - 1)
            print(iVVLabels)

            iSVi = copy.deepcopy(irisSetosa)
            iSVi.data.extend(irisVirginica.data)
            # print(iSVi.data)
            iSViLabels = iSVi.col(iSVi.cols -1)
            print(iSViLabels)
            ...
            # start_time = time.time()
            # learnerVer.train(, labels)
            # learnerVir.train(            )
            # learnerSet.train(            )
            # elapsed_time = time.time() - start_time
            # print("Time to train (in seconds): {}".format(elapsed_time))
            # accuracy = learner1.measure_accuracy(features, labels, confusion)
            # print("Training set accuracy: " + str(accuracy))
            #
            # if print_confusion_matrix:
            #     print("\nConfusion matrix: (Row=target value, Col=predicted value)")
